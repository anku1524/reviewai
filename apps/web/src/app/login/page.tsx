...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to fetch error coming while login");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="min-h-screen bg-[#0B0F19] relative overflow-hidden flex items-center justify-center px-6 py-24 font-sans">
      ...
      <main className="w-full max-w-sm glass-card border border-slate-800 rounded-2xl p-8 shadow-2xl relative z-10">
        ...
        <div className="flex flex-col items-center gap-2 mb-6">
          ...
          <h1 className="text-xl font-extrabold text-white mt-4">Welcome Back</h1>
          <p className="text-xs text-slate-400">Enter your credentials to access your growth dashboard</p>
        </div>
